...
class CartSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cart
        fields = ['id', 'product_id', 'product', 'items','quantity','total_price','is_deleted', 'user', 'created_at', 'updated_at']
        read_only_fields = ('created_at', 'updated_at')
    ...
    def create(self, validated_data):
        ...
        if CartItem.objects.filter(product=validated_data['product'], cart__user__id=validated_data['user'].id).exists():

            cart_item = CartItem.objects.get(product=validated_data['product'], cart__user__id=validated_data['user'].id)
            cart_item.quantity +=validated_data['quantity']
            cart_item.save()
            return cart
        cart_item= CartItem.objects.create(cart=cart, product=validated_data['product'], quantity=validated_data['quantity'])
        return cart
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for item in cart.items.all():
            order, created = Order.objects.get_or_create(user=validated_data['user'], product=item.product, total_price=item.total_price)
...
class OneOrderSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        order , created = Order.objects.get_or_create(user=validated_data['user'], product=product, total_price=quantity*product.price)
